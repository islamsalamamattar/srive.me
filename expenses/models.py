...
class SubscriptionManager(models.Manager):
    ...
    def to_be_paid_manual(self, user):
        subscriptions = self.objects.filter(user=user, autopay=False, date__lte=datetime.date.today())
        return subscriptions
    ## auto paying >> creating expense for due subscription set to autopay
    def auto_pay(self, user):
        subscriptions = self.objects.filter(user=user, autopay=True, date__lte=datetime.date.today())
